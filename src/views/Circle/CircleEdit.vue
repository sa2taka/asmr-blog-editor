<template>
  <div class="container">
    ...
    <v-form v-model="valid" id="form" ref="form" class="mx-auto">
      <v-text-field
        v-model="name"
        label="name"
        required
      ...
      <div class="mt-2 mt-4 d-flex justify-center">
...
import { Circle, SocialInfo } from '@/types/entry';
...
export default class CircleEdit extends Vue {
  name = '';
  slug = '';
  id = '';
  ...
  created() {
    ...
    fetchCircleBySlug(slug).then(result => {
  ...
  add() {
    ...
    if (this.isEdit && this.id) {
      updateCircle(this.id, data).then(back);
    } else {
      addCircle(data).then(back);
    }
  }

  remove() {
    if (window.confirm('削除しますがよろしいですか')) {
      deleteCircle(this.id).then(() => this.$router.back());
    }
  }
}
</script>
